...
async def login(
    ...
    user = db.scalar(select(Admin).where(Admin.name == username))

    if not user:
        user = db.scalar(select(User).where(User.name == username))
    if not user and not verify_pass(password, user.password):
        raise HTTPException(
    ...
    access_token = create_token(data=token_data, expires_delta=timedelta(hours=100))
    return {"access_token": access_token, "token_type": "bearer"}


""" admin = db.scalar(select(Admin).where(Admin.name == username))
if not admin or not verify_pass(password, admin.password):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Неверный логин или пароль"
        )
    else:
